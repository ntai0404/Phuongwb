...
export default function SavedPage() {
  ...
  const { data: savedArticles, isLoading, refetch } = useQuery({
    queryKey: ['saved-articles', user?.id || 1],
    queryFn: async () => {
      return await articlesApi.getSavedArticles();
    },
    refetchOnWindowFocus: true,
    staleTime: 0, // Always refetch
  });

  const filteredArticles = savedArticles?.filter((article: Article) =>
        ...
        <main className="flex-1 p-6">
          <div className="max-w-7xl mx-auto">
            <div className="mb-6 flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Bài viết đã lưu</h1>
                <p className="text-gray-600">Danh sách các bài viết bạn đã lưu để đọc sau</p>
              </div>
            </div>
